...
CREATE INDEX idx_ai_recommendations_expires ON public.ai_recommendations(expires_at) WHERE expires_at > NOW();
...
CREATE INDEX idx_ai_model_configs_active ON public.ai_model_configs(is_active) WHERE is_active = true;
CREATE INDEX idx_ai_training_data_processed ON public.ai_training_data(is_processed) WHERE is_processed = false;
CREATE INDEX idx_smart_automation_rules_active ON public.smart_automation_rules(is_active) WHERE is_active = true;
